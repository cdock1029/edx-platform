...
class TestAccountRetirementsByStatusAndDate(RetirementTestCase):
    ...
    def test_date_filter(self):
        ...
        """
        retirements = []
        pending_state = RetirementState.objects.get(state_name='COMPLETE')

        # Create retirements for the last 10 days
        for days_back in range(0, 10):
            create_datetime = datetime.datetime.now(pytz.UTC) - datetime.timedelta(days=days_back)
            ret = self._create_retirement(state=pending_state, create_datetime=create_datetime)
        ...
        for days_back in range(1, 11):
            ...
            start_date = datetime.datetime.now(pytz.UTC) - datetime.timedelta(days=days_back - 1)
            self.assert_status_and_user_list(
                retirement_dicts,
